...
export default function NoteEditorView() {
  ...
  return (
    ...
    bg-beige-100 text-slate-900 antialiased dark:bg-slate-900 dark:text-slate-50">
      {/* Sidebar */}
      <aside
        className={`
      ...
      >
        ...
        <button
          type="button"
          className="mx-4 mb-6 flex items-center justify-center gap-2 border border-slate-300 
          bg-beige-200 px-4 py-2 font-ui text-sm font-medium text-slate-800 transition-colors 
          hover:bg-slate-300/60 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600"
        >
          <Plus size={18} />
          New Note
        </button>
        ...
        <nav className="flex flex-1 flex-col gap-1 overflow-y-auto px-2">
          ...
          <div className="flex cursor-pointer items-center gap-3 px-3 py-2 font-ui 
          text-sm text-slate-600 transition-colors 
          hover:bg-slate-200/60 dark:text-slate-400 dark:hover:bg-slate-700/60">
            <Archive size={18} />
            <span>Archive</span>
          </div>
        </nav>
        ...
        <div className="mt-auto flex flex-col gap-1 border-t border-slate-300 px-2 pt-4 dark:border-slate-700">
          {sidebarFooter.map((item) => (
            <div
              key={item.label}
              className="flex cursor-pointer items-center gap-3 px-3 py-2 font-ui text-sm 
              text-slate-600 transition-colors hover:bg-slate-200/60 dark:text-slate-400 dark:hover:bg-slate-700/60"
            >
              <item.icon size={18} />
              <span>{item.label}</span>
            </div>
      ...
      <main className="flex h-full flex-1 flex-col bg-beige-100 dark:bg-slate-900">
        <header className="sticky top-0 z-20 flex h-14 w-full items-center justify-between 
        border-b border-slate-300 bg-beige-100 px-4 dark:border-slate-700 dark:bg-slate-900 md:px-6">
        ...
        <div className="flex w-full flex-1 justify-center overflow-y-auto py-8 sm:py-10">
          <article className="w-full max-w-[800px] px-4 sm:px-6">
            <header className="mb-6 border-b border-slate-300 pb-4 dark:border-slate-700">
              ...
              <div className="flex flex-wrap gap-2">
                {note.tags.map((tag) => (
                  <span
                    key={tag.label}
                    className="rounded-none border border-slate-400/60 bg-slate-200/60 px-2 py-1 
                    font-mono text-xs text-slate-700 dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-300"
                  >
                    {tag.label}
                  </span>
                ))}
            ...
            <div className="space-y-6 font-serif text-base leading-relaxed text-slate-700 dark:text-slate-300 sm:text-lg">
              ...
              <blockquote className="border-l-4 border-slate-400 pl-4 italic text-slate-500 
              ...
              <h2 className="mt-8 mb-4 inline-block border-b border-slate-300/50 pb-2 font-serif text-2xl font-semibold 
